...
test('should roll various dice', () => {
    const roller = genesysRoller(makeRng(1, 3, 5, 3, 4, 6, 7), 'gen');
    ...
    expect(result[2].die).toBe(Dice.PROFICIENCY);
    expect(result[2].face).toBe(Faces.ADVANTAGE);
    expect(result[3].die).toBe(Dice.SETBACK);
    expect(result[3].face).toBe(Faces.FAILURE);
    expect(result[4].die).toBe(Dice.DIFFICULTY);
    expect(result[4].face).toBe(Faces.THREAT);
